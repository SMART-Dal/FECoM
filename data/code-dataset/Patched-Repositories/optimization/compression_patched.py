...
class CustomConv2D(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        outputs = custom_method(
        tf.nn.convolution(inputs, self.kernel, strides=self.strides, padding=self.padding), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.nn.convolution(*args, **kwargs)', method_object=None, object_signature=None, function_args=[eval('inputs'), eval('self.kernel')], function_kwargs={'strides': eval('self.strides'), 'padding': eval('self.padding')})
        outputs = custom_method(
        tf.nn.bias_add(outputs, self.bias), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.nn.bias_add(*args)', method_object=None, object_signature=None, function_args=[eval('outputs'), eval('self.bias')], function_kwargs={})
        return custom_method(
        tf.nn.leaky_relu(outputs), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.nn.leaky_relu(*args)', method_object=None, object_signature=None, function_args=[eval('outputs')], function_kwargs={})
classifier = custom_method(
tf.keras.Sequential([CustomConv2D(20, 5, strides=2, name='conv_1'), CustomConv2D(50, 5, strides=2, name='conv_2'), tf.keras.layers.Flatten(), CustomDense(500, name='fc_1'), CustomDense(10, name='fc_2')], name='classifier'), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.keras.Sequential(*args, **kwargs)', method_object=None, object_signature=None, function_args=[eval('[\n    CustomConv2D(20, 5, strides=2, name="conv_1"),\n    CustomConv2D(50, 5, strides=2, name="conv_2"),\n    tf.keras.layers.Flatten(),\n    CustomDense(500, name="fc_1"),\n    CustomDense(10, name="fc_2"),\n]')], function_kwargs={'name': eval('"classifier"')})

def normalize_img(image, label):
    """Normalizes images: `uint8` -> `float32`."""
    return (custom_method(
    tf.cast(image, tf.float32), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.cast(*args)', method_object=None, object_signature=None, function_args=[eval('image'), eval('tf.float32')], function_kwargs={}) / 255.0, label)
(training_dataset, validation_dataset) = custom_method(
tfds.load('mnist', split=['train', 'test'], shuffle_files=True, as_supervised=True, with_info=False), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tfds.load(*args, **kwargs)', method_object=None, object_signature=None, function_args=[eval('"mnist"')], function_kwargs={'split': eval('["train", "test"]'), 'shuffle_files': eval('True'), 'as_supervised': eval('True'), 'with_info': eval('False')})
...
_ = custom_method(
tf.linspace(-5.0, 5.0, 501), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.linspace(*args)', method_object=None, object_signature=None, function_args=[eval('-5.'), eval('5.'), eval('501')], function_kwargs={})
...
class PowerLawRegularizer(tf.keras.regularizers.Regularizer):
    ...
    def __call__(self, variable):
        em = custom_method(
        tfc.PowerLawEntropyModel(coding_rank=variable.shape.rank), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tfc.PowerLawEntropyModel(**kwargs)', method_object=None, object_signature=None, function_args=[], function_kwargs={'coding_rank': eval('variable.shape.rank')})
        return self.lmbda * custom_method(
        em.penalty(variable), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='obj.penalty(*args)', method_object=eval('em'), object_signature='tfc.PowerLawEntropyModel', function_args=[eval('variable')], function_kwargs={}, custom_class=None)
regularizer = PowerLawRegularizer(lmbda=2.0 / classifier.count_params())

def quantize(latent, log_step):
    step = custom_method(
    tf.exp(log_step), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.exp(*args)', method_object=None, object_signature=None, function_args=[eval('log_step')], function_kwargs={})
    return custom_method(
    tfc.round_st(latent / step), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tfc.round_st(*args)', method_object=None, object_signature=None, function_args=[eval('latent / step')], function_kwargs={}) * step
...
class CompressibleDense(CustomDense):
    ...
    def build(self, input_shape, other=None):
        ...
        tf.Variable(tf.cast(kernel_log_step, self.variable_dtype), name='kernel_log_step'), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.Variable(*args, **kwargs)', method_object=None, object_signature=None, function_args=[eval('tf.cast(kernel_log_step, self.variable_dtype)')], function_kwargs={'name': eval('"kernel_log_step"')})
        self.bias_log_step = custom_method(
        tf.Variable(tf.cast(bias_log_step, self.variable_dtype), name='bias_log_step'), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.Variable(*args, **kwargs)', method_object=None, object_signature=None, function_args=[eval('tf.cast(bias_log_step, self.variable_dtype)')], function_kwargs={'name': eval('"bias_log_step"')})
    ...
    def kernel(self, kernel):
        ...
        tf.Variable(kernel, name='kernel_latent'), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.Variable(*args, **kwargs)', method_object=None, object_signature=None, function_args=[eval('kernel')], function_kwargs={'name': eval('"kernel_latent"')})
...
class CompressibleConv2D(CustomConv2D):
    ...
    def build(self, input_shape, other=None):
        ...
        if other is not None and hasattr(other, 'kernel_log_step'):
            ...
            tf.fill(self.kernel_latent.shape[2:], -4.0), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.fill(*args)', method_object=None, object_signature=None, function_args=[eval('self.kernel_latent.shape[2:]'), eval('-4.')], function_kwargs={})
            bias_log_step = -4.0
        self.kernel_log_step = custom_method(
        tf.Variable(tf.cast(kernel_log_step, self.variable_dtype), name='kernel_log_step'), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.Variable(*args, **kwargs)', method_object=None, object_signature=None, function_args=[eval('tf.cast(kernel_log_step, self.variable_dtype)')], function_kwargs={'name': eval('"kernel_log_step"')})
        self.bias_log_step = custom_method(
    ...
    def kernel(self, kernel):
        ...
        self.kernel_latent = custom_method(
        tf.Variable(kernel_rdft, name='kernel_latent'), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.Variable(*args, **kwargs)', method_object=None, object_signature=None, function_args=[eval('kernel_rdft')], function_kwargs={'name': eval('"kernel_latent"')})
...
def make_mnist_classifier(regularizer):
    return custom_method(
    tf.keras.Sequential([CompressibleConv2D(regularizer, 20, 5, strides=2, name='conv_1'), CompressibleConv2D(regularizer, 50, 5, strides=2, name='conv_2'), tf.keras.layers.Flatten(), CompressibleDense(regularizer, 500, name='fc_1'), CompressibleDense(regularizer, 10, name='fc_2')], name='classifier'), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.keras.Sequential(*args, **kwargs)', method_object=None, object_signature=None, function_args=[eval('[\n      CompressibleConv2D(regularizer, 20, 5, strides=2, name="conv_1"),\n      CompressibleConv2D(regularizer, 50, 5, strides=2, name="conv_2"),\n      tf.keras.layers.Flatten(),\n      CompressibleDense(regularizer, 500, name="fc_1"),\n      CompressibleDense(regularizer, 10, name="fc_2"),\n  ]')], function_kwargs={'name': eval('"classifier"')})
...
def compress_latent(latent, log_step, name):
    em = custom_method(
    tfc.PowerLawEntropyModel(latent.shape.rank), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tfc.PowerLawEntropyModel(*args)', method_object=None, object_signature=None, function_args=[eval('latent.shape.rank')], function_kwargs={})
    compressed = custom_method(
    em.compress(latent / tf.exp(log_step)), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='obj.compress(*args)', method_object=eval('em'), object_signature='tfc.PowerLawEntropyModel', function_args=[eval('latent / tf.exp(log_step)')], function_kwargs={}, custom_class=None)
    compressed = custom_method(
    tf.Variable(compressed, name=f'{name}_compressed'), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.Variable(*args, **kwargs)', method_object=None, object_signature=None, function_args=[eval('compressed')], function_kwargs={'name': eval('f"{name}_compressed"')})
    log_step = custom_method(
    tf.cast(log_step, tf.float16), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.cast(*args)', method_object=None, object_signature=None, function_args=[eval('log_step'), eval('tf.float16')], function_kwargs={})
    log_step = custom_method(
    tf.Variable(log_step, name=f'{name}_log_step'), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.Variable(*args, **kwargs)', method_object=None, object_signature=None, function_args=[eval('log_step')], function_kwargs={'name': eval('f"{name}_log_step"')})
    return (compressed, log_step)

def decompress_latent(compressed, shape, log_step):
    latent = custom_method(
    tfc.PowerLawEntropyModel(len(shape)).decompress(compressed, shape), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tfc.PowerLawEntropyModel(len(shape)).decompress(*args)', method_object=None, object_signature=None, function_args=[eval('compressed'), eval('shape')], function_kwargs={})
    step = custom_method(
    tf.exp(tf.cast(log_step, latent.dtype)), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.exp(*args)', method_object=None, object_signature=None, function_args=[eval('tf.cast(log_step, latent.dtype)')], function_kwargs={})
    return latent * step
...
def compress_and_evaluate_model(lmbda):
    ...
    compressed_classifier = custom_method(
    tf.keras.models.load_model('/tmp/compressed_classifier'), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.keras.models.load_model(*args)', method_object=None, object_signature=None, function_args=[eval('"/tmp/compressed_classifier"')], function_kwargs={})
    custom_method(
    compressed_classifier.compile(metrics=[tf.keras.metrics.SparseCategoricalAccuracy()]), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='obj.compile(**kwargs)', method_object=eval('compressed_classifier'), object_signature='tf.keras.models.clone_model', function_args=[], function_kwargs={'metrics': eval('[tf.keras.metrics.SparseCategoricalAccuracy()]')}, custom_class=None)
    (_, compressed_accuracy) = custom_method(
    compressed_classifier.evaluate(validation_dataset.batch(128), verbose=0), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='obj.evaluate(*args, **kwargs)', method_object=eval('compressed_classifier'), object_signature='tf.keras.models.clone_model', function_args=[eval('validation_dataset.batch(128)')], function_kwargs={'verbose': eval('0')}, custom_class=None)
    print()
...
tf.convert_to_tensor(metrics, tf.float32), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.convert_to_tensor(*args)', method_object=None, object_signature=None, function_args=[eval('metrics'), eval('tf.float32')], function_kwargs={})
...
tf.transpose(metrics), imports='import tensorflow_datasets as tfds;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import os;import shutil;import tensorflow as tf', function_to_run='tf.transpose(*args)', method_object=None, object_signature=None, function_args=[eval('metrics')], function_kwargs={})
sizes /= 1024
zip_sizes /= 1024
