...
def input_fn(features, labels, training=True, batch_size=256):
    ...
    tf.data.Dataset.from_tensor_slices((dict(features), labels)), imports='import pandas as pd;import tensorflow as tf', function_to_run='tf.data.Dataset.from_tensor_slices(*args)', method_object=None, function_args=[eval('(dict(features), labels)')], function_kwargs={}, max_wait_secs=0)
    if training:
        dataset = custom_method(
        dataset.shuffle(1000).repeat(), imports='import pandas as pd;import tensorflow as tf', function_to_run='obj.shuffle(1000).repeat()', method_object=eval('dataset'), function_args=[], function_kwargs={}, max_wait_secs=0, custom_class=None)
    return custom_method(
    dataset.batch(batch_size), imports='import pandas as pd;import tensorflow as tf', function_to_run='obj.batch(*args)', method_object=eval('dataset'), function_args=[eval('batch_size')], function_kwargs={}, max_wait_secs=0, custom_class=None)
my_feature_columns = []
for key in train.keys():
    my_feature_columns.append(tf.feature_column.numeric_column(key=key))
classifier = custom_method(
tf.estimator.DNNClassifier(feature_columns=my_feature_columns, hidden_units=[30, 10], n_classes=3), imports='import pandas as pd;import tensorflow as tf', function_to_run='tf.estimator.DNNClassifier(**kwargs)', method_object=None, function_args=[], function_kwargs={'feature_columns': eval('my_feature_columns'), 'hidden_units': eval('[30, 10]'), 'n_classes': eval('3')}, max_wait_secs=0)
custom_method(
classifier.train(input_fn=lambda : input_fn(train, train_y, training=True), steps=5000), imports='import pandas as pd;import tensorflow as tf', function_to_run='obj.train(**kwargs)', method_object=eval('classifier'), function_args=[], function_kwargs={'input_fn': eval('lambda: input_fn(train, train_y, training=True)'), 'steps': eval('5000')}, max_wait_secs=0, custom_class=None)
eval_result = custom_method(
classifier.evaluate(input_fn=lambda : input_fn(test, test_y, training=False)), imports='import pandas as pd;import tensorflow as tf', function_to_run='obj.evaluate(**kwargs)', method_object=eval('classifier'), function_args=[], function_kwargs={'input_fn': eval('lambda: input_fn(test, test_y, training=False)')}, max_wait_secs=0, custom_class=None)
...
def input_fn(features, batch_size=256):
    ...
    return custom_method(
    tf.data.Dataset.from_tensor_slices(dict(features)).batch(batch_size), imports='import pandas as pd;import tensorflow as tf', function_to_run='tf.data.Dataset.from_tensor_slices(dict(features)).batch(*args)', method_object=None, function_args=[eval('batch_size')], function_kwargs={}, max_wait_secs=0)
predictions = custom_method(
classifier.predict(input_fn=lambda : input_fn(predict_x)), imports='import pandas as pd;import tensorflow as tf', function_to_run='obj.predict(**kwargs)', method_object=eval('classifier'), function_args=[], function_kwargs={'input_fn': eval('lambda: input_fn(predict_x)')}, max_wait_secs=0, custom_class=None)
