...
def input_fn(features, labels, training=True, batch_size=256):
    ...
    if training:
        dataset = custom_method(
        dataset.shuffle(1000).repeat(), imports='import pandas as pd;import tensorflow as tf', function_to_run='obj.shuffle(1000).repeat()', method_object=eval('dataset'), object_signature='tf.data.Dataset.from_tensor_slices', function_args=[], function_kwargs={}, custom_class=None)
    return custom_method(
    dataset.batch(batch_size), imports='import pandas as pd;import tensorflow as tf', function_to_run='obj.batch(*args)', method_object=eval('dataset'), object_signature='tf.data.Dataset.from_tensor_slices', function_args=[eval('batch_size')], function_kwargs={}, custom_class=None)
my_feature_columns = []
...
classifier.evaluate(input_fn=lambda : input_fn(test, test_y, training=False)), imports='import pandas as pd;import tensorflow as tf', function_to_run='obj.evaluate(**kwargs)', method_object=eval('classifier'), object_signature='tf.estimator.DNNClassifier', function_args=[], function_kwargs={'input_fn': eval('lambda: input_fn(test, test_y, training=False)')}, custom_class=None)
