...
test_dataset = custom_method(
test_dataset.batch(BATCH_SIZE), imports='import tensorflow as tf;import numpy as np', function_to_run='obj.batch(*args)', method_object=eval('test_dataset'), object_signature='tf.data.Dataset.from_tensor_slices', function_args=[eval('BATCH_SIZE')], function_kwargs={}, custom_class=None)
model = custom_method(
...
custom_method(
model.compile(optimizer=tf.keras.optimizers.RMSprop(), loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True), metrics=['sparse_categorical_accuracy']), imports='import tensorflow as tf;import numpy as np', function_to_run='obj.compile(**kwargs)', method_object=eval('model'), object_signature='tf.keras.Sequential', function_args=[], function_kwargs={'optimizer': eval('tf.keras.optimizers.RMSprop()'), 'loss': eval('tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)'), 'metrics': eval("['sparse_categorical_accuracy']")}, custom_class=None)
custom_method(
model.fit(train_dataset, epochs=10), imports='import tensorflow as tf;import numpy as np', function_to_run='obj.fit(*args, **kwargs)', method_object=eval('model'), object_signature='tf.keras.Sequential', function_args=[eval('train_dataset')], function_kwargs={'epochs': eval('10')}, custom_class=None)
custom_method(
model.evaluate(test_dataset), imports='import tensorflow as tf;import numpy as np', function_to_run='obj.evaluate(*args)', method_object=eval('model'), object_signature='tf.keras.Sequential', function_args=[eval('test_dataset')], function_kwargs={}, custom_class=None)
