...
pretrained_model.predict(image), imports='import matplotlib as mpl;import matplotlib.pyplot as plt;import tensorflow as tf', function_to_run='obj.predict(*args)', method_object=eval('pretrained_model'), object_signature='tf.keras.applications.MobileNetV2', function_args=[eval('image')], function_kwargs={}, custom_class=None)
plt.figure()
...
def create_adversarial_pattern(input_image, input_label):
    ...
    tf.GradientTape(), imports='import matplotlib as mpl;import matplotlib.pyplot as plt;import tensorflow as tf', function_to_run='tf.GradientTape()', method_object=None, object_signature=None, function_args=[], function_kwargs={}) as tape:
        ...
        pretrained_model(input_image), imports='import matplotlib as mpl;import matplotlib.pyplot as plt;import tensorflow as tf', function_to_run='obj(*args)', method_object=eval('pretrained_model'), object_signature='tf.keras.applications.MobileNetV2', function_args=[eval('input_image')], function_kwargs={}, custom_class=None)
        loss = custom_method(
        loss_object(input_label, prediction), imports='import matplotlib as mpl;import matplotlib.pyplot as plt;import tensorflow as tf', function_to_run='obj(*args)', method_object=eval('loss_object'), object_signature='tf.keras.losses.CategoricalCrossentropy', function_args=[eval('input_label'), eval('prediction')], function_kwargs={}, custom_class=None)
