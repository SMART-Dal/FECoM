...
class MNISTCompressionTrainer(tf.keras.Model):
    ...
    def __init__(self, latent_dims):
        ...
        tf.Variable(tf.zeros((latent_dims,))), imports='import tensorflow_datasets as tfds;import os;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import tensorflow as tf', function_to_run='tf.Variable(*args)', method_object=None, object_signature=None, function_args=[eval('tf.zeros((latent_dims,))')], function_kwargs={})
    ...
    def call(self, x, training):
        ...
        x = custom_method(
        tf.reshape(x, (-1, 28, 28, 1)), imports='import tensorflow_datasets as tfds;import os;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import tensorflow as tf', function_to_run='tf.reshape(*args)', method_object=None, object_signature=None, function_args=[eval('x'), eval('(-1, 28, 28, 1)')], function_kwargs={})
        y = self.analysis_transform(x)
        entropy_model = tfc.ContinuousBatchedEntropyModel(self.prior, coding_rank=1, compression=False)
        (y_tilde, rate) = entropy_model(y, training=training)
        x_tilde = self.synthesis_transform(y_tilde)
        rate = custom_method(
        tf.reduce_mean(rate), imports='import tensorflow_datasets as tfds;import os;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import tensorflow as tf', function_to_run='tf.reduce_mean(*args)', method_object=None, object_signature=None, function_args=[eval('rate')], function_kwargs={})
        distortion = custom_method(
        tf.reduce_mean(abs(x - x_tilde)), imports='import tensorflow_datasets as tfds;import os;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import tensorflow as tf', function_to_run='tf.reduce_mean(*args)', method_object=None, object_signature=None, function_args=[eval('abs(x - x_tilde)')], function_kwargs={})
...
def make_mnist_compression_trainer(lmbda, latent_dims=50):
    ...
    custom_method(
    trainer.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.001), loss=dict(rate=pass_through_loss, distortion=pass_through_loss), metrics=dict(rate=pass_through_loss, distortion=pass_through_loss), loss_weights=dict(rate=1.0, distortion=lmbda)), imports='import tensorflow_datasets as tfds;import os;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import tensorflow as tf', function_to_run='obj.compile(**kwargs)', method_object=eval('trainer'), object_signature='MNISTCompressionTrainer(latent_dims)', function_args=[], function_kwargs={'optimizer': eval('tf.keras.optimizers.Adam(learning_rate=1e-3)'), 'loss': eval('dict(rate=pass_through_loss, distortion=pass_through_loss)'), 'metrics': eval('dict(rate=pass_through_loss, distortion=pass_through_loss)'), 'loss_weights': eval('dict(rate=1., distortion=lmbda)')}, custom_class='class MNISTCompressionTrainer(tf.keras.Model):\n  """Model that trains a compressor/decompressor for MNIST."""\n\n  def __init__(self, latent_dims):\n    super().__init__()\n    self.analysis_transform = make_analysis_transform(latent_dims)\n    self.synthesis_transform = make_synthesis_transform()\n    self.prior_log_scales = tf.Variable(tf.zeros((latent_dims,)))\n\n  @property\n  def prior(self):\n    return tfc.NoisyLogistic(loc=0., scale=tf.exp(self.prior_log_scales))\n\n  def call(self, x, training):\n    """Computes rate and distortion losses."""\n    x = tf.cast(x, self.compute_dtype) / 255.\n    x = tf.reshape(x, (-1, 28, 28, 1))\n\n    y = self.analysis_transform(x)\n    entropy_model = tfc.ContinuousBatchedEntropyModel(\n        self.prior, coding_rank=1, compression=False)\n    y_tilde, rate = entropy_model(y, training=training)\n    x_tilde = self.synthesis_transform(y_tilde)\n\n    rate = tf.reduce_mean(rate)\n\n    distortion = tf.reduce_mean(abs(x - x_tilde))\n\n    return dict(rate=rate, distortion=distortion)')
    return trainer
...
def train_mnist_model(lmbda):
    ...
    custom_method(
    trainer.fit(training_dataset.map(add_rd_targets).batch(128).prefetch(8), epochs=15, validation_data=validation_dataset.map(add_rd_targets).batch(128).cache(), validation_freq=1, verbose=1), imports='import tensorflow_datasets as tfds;import os;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import tensorflow as tf', function_to_run='obj.fit(*args, **kwargs)', method_object=eval('trainer'), object_signature='MNISTCompressionTrainer(latent_dims)', function_args=[eval('training_dataset.map(add_rd_targets).batch(128).prefetch(8)')], function_kwargs={'epochs': eval('15'), 'validation_data': eval('validation_dataset.map(add_rd_targets).batch(128).cache()'), 'validation_freq': eval('1'), 'verbose': eval('1')}, custom_class='class MNISTCompressionTrainer(tf.keras.Model):\n  """Model that trains a compressor/decompressor for MNIST."""\n\n  def __init__(self, latent_dims):\n    super().__init__()\n    self.analysis_transform = make_analysis_transform(latent_dims)\n    self.synthesis_transform = make_synthesis_transform()\n    self.prior_log_scales = tf.Variable(tf.zeros((latent_dims,)))\n\n  @property\n  def prior(self):\n    return tfc.NoisyLogistic(loc=0., scale=tf.exp(self.prior_log_scales))\n\n  def call(self, x, training):\n    """Computes rate and distortion losses."""\n    x = tf.cast(x, self.compute_dtype) / 255.\n    x = tf.reshape(x, (-1, 28, 28, 1))\n\n    y = self.analysis_transform(x)\n    entropy_model = tfc.ContinuousBatchedEntropyModel(\n        self.prior, coding_rank=1, compression=False)\n    y_tilde, rate = entropy_model(y, training=training)\n    x_tilde = self.synthesis_transform(y_tilde)\n\n    rate = tf.reduce_mean(rate)\n\n    distortion = tf.reduce_mean(abs(x - x_tilde))\n\n    return dict(rate=rate, distortion=distortion)')
    return trainer
...
compressor(originals), imports='import tensorflow_datasets as tfds;import os;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import tensorflow as tf', function_to_run='obj(*args)', method_object=eval('compressor'), object_signature=None, function_args=[eval('originals')], function_kwargs={}, custom_class='class MNISTCompressor(tf.keras.Model):\n  """Compresses MNIST images to strings."""\n\n  def __init__(self, analysis_transform, entropy_model):\n    super().__init__()\n    self.analysis_transform = analysis_transform\n    self.entropy_model = entropy_model\n\n  def call(self, x):\n    x = tf.cast(x, self.compute_dtype) / 255.\n    y = self.analysis_transform(x)\n    _, bits = self.entropy_model(y, training=False)\n    return self.entropy_model.compress(y), bits')
...
tf.constant([os.urandom(8) for _ in range(16)]), imports='import tensorflow_datasets as tfds;import os;import matplotlib.pyplot as plt;import tensorflow_compression as tfc;import tensorflow as tf', function_to_run='tf.constant(*args)', method_object=None, object_signature=None, function_args=[eval('[os.urandom(8) for _ in range(16)]')], function_kwargs={})
