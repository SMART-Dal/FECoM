...
custom_method(
layer(tf.zeros([10, 5])), imports='import tensorflow as tf', function_to_run='obj(*args)', method_object=eval('layer'), object_signature='tf.keras.layers.Dense', function_args=[eval('tf.zeros([10, 5])')], function_kwargs={}, custom_class='class MyDenseLayer(tf.keras.layers.Layer):\n  def __init__(self, num_outputs):\n    super(MyDenseLayer, self).__init__()\n    self.num_outputs = num_outputs\n\n  def build(self, input_shape):\n    self.kernel = self.add_weight("kernel",\n                                  shape=[int(input_shape[-1]),\n                                         self.num_outputs])\n\n  def call(self, inputs):\n    return tf.matmul(inputs, self.kernel)')
layer.variables
...
_ = custom_method(
block(tf.zeros([1, 2, 3, 3])), imports='import tensorflow as tf', function_to_run='obj(*args)', method_object=eval('block'), object_signature='ResnetIdentityBlock', function_args=[eval('tf.zeros([1, 2, 3, 3])')], function_kwargs={}, custom_class="class ResnetIdentityBlock(tf.keras.Model):\n  def __init__(self, kernel_size, filters):\n    super(ResnetIdentityBlock, self).__init__(name='')\n    filters1, filters2, filters3 = filters\n\n    self.conv2a = tf.keras.layers.Conv2D(filters1, (1, 1))\n    self.bn2a = tf.keras.layers.BatchNormalization()\n\n    self.conv2b = tf.keras.layers.Conv2D(filters2, kernel_size, padding='same')\n    self.bn2b = tf.keras.layers.BatchNormalization()\n\n    self.conv2c = tf.keras.layers.Conv2D(filters3, (1, 1))\n    self.bn2c = tf.keras.layers.BatchNormalization()\n\n  def call(self, input_tensor, training=False):\n    x = self.conv2a(input_tensor)\n    x = self.bn2a(x, training=training)\n    x = tf.nn.relu(x)\n\n    x = self.conv2b(x)\n    x = self.bn2b(x, training=training)\n    x = tf.nn.relu(x)\n\n    x = self.conv2c(x)\n    x = self.bn2c(x, training=training)\n\n    x += input_tensor\n    return tf.nn.relu(x)")
block.layers
len(block.variables)
custom_method(
block.summary(), imports='import tensorflow as tf', function_to_run='obj.summary()', method_object=eval('block'), object_signature='ResnetIdentityBlock', function_args=[], function_kwargs={}, custom_class="class ResnetIdentityBlock(tf.keras.Model):\n  def __init__(self, kernel_size, filters):\n    super(ResnetIdentityBlock, self).__init__(name='')\n    filters1, filters2, filters3 = filters\n\n    self.conv2a = tf.keras.layers.Conv2D(filters1, (1, 1))\n    self.bn2a = tf.keras.layers.BatchNormalization()\n\n    self.conv2b = tf.keras.layers.Conv2D(filters2, kernel_size, padding='same')\n    self.bn2b = tf.keras.layers.BatchNormalization()\n\n    self.conv2c = tf.keras.layers.Conv2D(filters3, (1, 1))\n    self.bn2c = tf.keras.layers.BatchNormalization()\n\n  def call(self, input_tensor, training=False):\n    x = self.conv2a(input_tensor)\n    x = self.bn2a(x, training=training)\n    x = tf.nn.relu(x)\n\n    x = self.conv2b(x)\n    x = self.bn2b(x, training=training)\n    x = tf.nn.relu(x)\n\n    x = self.conv2c(x)\n    x = self.bn2c(x, training=training)\n\n    x += input_tensor\n    return tf.nn.relu(x)")
...
custom_method(
my_seq(tf.zeros([1, 2, 3, 3])), imports='import tensorflow as tf', function_to_run='obj(*args)', method_object=eval('my_seq'), object_signature='tf.keras.Sequential', function_args=[eval('tf.zeros([1, 2, 3, 3])')], function_kwargs={}, custom_class=None)
custom_method(
my_seq.summary(), imports='import tensorflow as tf', function_to_run='obj.summary()', method_object=eval('my_seq'), object_signature='tf.keras.Sequential', function_args=[], function_kwargs={}, custom_class=None)
