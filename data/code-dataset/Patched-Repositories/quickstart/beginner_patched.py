...
predictions
custom_method(
tf.nn.softmax(predictions).numpy(), imports='import tensorflow as tf', function_to_run='tf.nn.softmax(predictions).numpy()', method_object=None, object_signature=None, function_args=[], function_kwargs={}, max_wait_secs=0)
loss_fn = custom_method(
tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True), imports='import tensorflow as tf', function_to_run='tf.keras.losses.SparseCategoricalCrossentropy(**kwargs)', method_object=None, object_signature=None, function_args=[], function_kwargs={'from_logits': eval('True')}, max_wait_secs=0)
loss_fn(y_train[:1], predictions).numpy()
custom_method(
model.compile(optimizer='adam', loss=loss_fn, metrics=['accuracy']), imports='import tensorflow as tf', function_to_run='obj.compile(**kwargs)', method_object=eval('model'), object_signature='tf.keras.models.Sequential', function_args=[], function_kwargs={'optimizer': eval("'adam'"), 'loss': eval('loss_fn'), 'metrics': eval("['accuracy']")}, max_wait_secs=0, custom_class=None)
custom_method(
model.fit(x_train, y_train, epochs=5), imports='import tensorflow as tf', function_to_run='obj.fit(*args, **kwargs)', method_object=eval('model'), object_signature='tf.keras.models.Sequential', function_args=[eval('x_train'), eval('y_train')], function_kwargs={'epochs': eval('5')}, max_wait_secs=0, custom_class=None)
custom_method(
model.evaluate(x_test, y_test, verbose=2), imports='import tensorflow as tf', function_to_run='obj.evaluate(*args, **kwargs)', method_object=eval('model'), object_signature='tf.keras.models.Sequential', function_args=[eval('x_test'), eval('y_test')], function_kwargs={'verbose': eval('2')}, max_wait_secs=0, custom_class=None)
