...
tfds.load(name='imdb_reviews', split=('train[:60%]', 'train[60%:]', 'test'), as_supervised=True), imports='import tensorflow_hub as hub;import tensorflow as tf;import numpy as np;import tensorflow_datasets as tfds;import os', function_to_run='tfds.load(**kwargs)', method_object=None, object_signature=None, function_args=[], function_kwargs={'name': eval('"imdb_reviews"'), 'split': eval("('train[:60%]', 'train[60%:]', 'test')"), 'as_supervised': eval('True')})
...
custom_method(
model.add(hub_layer), imports='import tensorflow_hub as hub;import tensorflow as tf;import numpy as np;import tensorflow_datasets as tfds;import os', function_to_run='obj.add(*args)', method_object=eval('model'), object_signature='tf.keras.Sequential', function_args=[eval('hub_layer')], function_kwargs={}, custom_class=None)
custom_method(
...
custom_method(
model.summary(), imports='import tensorflow_hub as hub;import tensorflow as tf;import numpy as np;import tensorflow_datasets as tfds;import os', function_to_run='obj.summary()', method_object=eval('model'), object_signature='tf.keras.Sequential', function_args=[], function_kwargs={}, custom_class=None)
custom_method(
model.compile(optimizer='adam', loss=tf.keras.losses.BinaryCrossentropy(from_logits=True), metrics=['accuracy']), imports='import tensorflow_hub as hub;import tensorflow as tf;import numpy as np;import tensorflow_datasets as tfds;import os', function_to_run='obj.compile(**kwargs)', method_object=eval('model'), object_signature='tf.keras.Sequential', function_args=[], function_kwargs={'optimizer': eval("'adam'"), 'loss': eval('tf.keras.losses.BinaryCrossentropy(from_logits=True)'), 'metrics': eval("['accuracy']")}, custom_class=None)
history = custom_method(
model.fit(train_data.shuffle(10000).batch(512), epochs=10, validation_data=validation_data.batch(512), verbose=1), imports='import tensorflow_hub as hub;import tensorflow as tf;import numpy as np;import tensorflow_datasets as tfds;import os', function_to_run='obj.fit(*args, **kwargs)', method_object=eval('model'), object_signature='tf.keras.Sequential', function_args=[eval('train_data.shuffle(10000).batch(512)')], function_kwargs={'epochs': eval('10'), 'validation_data': eval('validation_data.batch(512)'), 'verbose': eval('1')}, custom_class=None)
results = custom_method(
model.evaluate(test_data.batch(512), verbose=2), imports='import tensorflow_hub as hub;import tensorflow as tf;import numpy as np;import tensorflow_datasets as tfds;import os', function_to_run='obj.evaluate(*args, **kwargs)', method_object=eval('model'), object_signature='tf.keras.Sequential', function_args=[eval('test_data.batch(512)')], function_kwargs={'verbose': eval('2')}, custom_class=None)
