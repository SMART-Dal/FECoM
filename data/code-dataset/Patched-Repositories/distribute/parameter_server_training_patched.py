...
dataset = custom_method(
dataset.prefetch(2), imports='import os;import random;import portpicker;import multiprocessing;import tensorflow as tf', function_to_run='obj.prefetch(*args)', method_object=eval('dataset'), object_signature='tf.data.Dataset.from_tensor_slices((x, y)).shuffle(10).repeat', function_args=[eval('2')], function_kwargs={}, custom_class=None)
with custom_method(
strategy.scope(), imports='import os;import random;import portpicker;import multiprocessing;import tensorflow as tf', function_to_run='obj.scope()', method_object=eval('strategy'), object_signature='tf.distribute.ParameterServerStrategy', function_args=[], function_kwargs={}, custom_class=None):
    model = custom_method(
...
strategy.scope(), imports='import os;import random;import portpicker;import multiprocessing;import tensorflow as tf', function_to_run='obj.scope()', method_object=eval('strategy'), object_signature='tf.distribute.ParameterServerStrategy', function_args=[], function_kwargs={}, custom_class=None):
    ...
    label_lookup_layer(raw_label_input), imports='import os;import random;import portpicker;import multiprocessing;import tensorflow as tf', function_to_run='obj(*args)', method_object=eval('label_lookup_layer'), object_signature='tf.keras.layers.StringLookup', function_args=[eval('raw_label_input')], function_kwargs={}, custom_class=None)
...
def dataset_fn(_):
    ...
    raw_dataset.map(lambda x: ({'features': feature_preprocess_stage(x['features'])}, label_preprocess_stage(x['label']))).shuffle(200).batch(32).repeat(), imports='import os;import random;import portpicker;import multiprocessing;import tensorflow as tf', function_to_run="raw_dataset.map(lambda obj: ({'features': feature_preprocess_stage(x['features'])}, label_preprocess_stage(x['label']))).shuffle(200).batch(32).repeat()", method_object=eval('x'), object_signature='tf.random.uniform', function_args=[], function_kwargs={}, custom_class=None)
...
def step_fn(iterator):
    ...
    strategy.run(replica_fn, args=(batch_data, labels)), imports='import os;import random;import portpicker;import multiprocessing;import tensorflow as tf', function_to_run='obj.run(*args, **kwargs)', method_object=eval('strategy'), object_signature='tf.distribute.ParameterServerStrategy', function_args=[eval('replica_fn')], function_kwargs={'args': eval('(batch_data, labels)')}, custom_class=None)
    return custom_method(
    strategy.reduce(tf.distribute.ReduceOp.SUM, losses, axis=None), imports='import os;import random;import portpicker;import multiprocessing;import tensorflow as tf', function_to_run='obj.reduce(*args, **kwargs)', method_object=eval('strategy'), object_signature='tf.distribute.ParameterServerStrategy', function_args=[eval('tf.distribute.ReduceOp.SUM'), eval('losses')], function_kwargs={'axis': eval('None')}, custom_class=None)
...
for i in range(num_epochs):
    custom_method(
    accuracy.reset_states(), imports='import os;import random;import portpicker;import multiprocessing;import tensorflow as tf', function_to_run='obj.reset_states()', method_object=eval('accuracy'), object_signature='tf.keras.metrics.Accuracy', function_args=[], function_kwargs={}, custom_class=None)
    for _ in range(steps_per_epoch):
        custom_method(
        coordinator.schedule(step_fn, args=(per_worker_iterator,)), imports='import os;import random;import portpicker;import multiprocessing;import tensorflow as tf', function_to_run='obj.schedule(*args, **kwargs)', method_object=eval('coordinator'), object_signature='tf.distribute.coordinator.ClusterCoordinator', function_args=[eval('step_fn')], function_kwargs={'args': eval('(per_worker_iterator,)')}, custom_class=None)
    custom_method(
    coordinator.join(), imports='import os;import random;import portpicker;import multiprocessing;import tensorflow as tf', function_to_run='obj.join()', method_object=eval('coordinator'), object_signature='tf.distribute.coordinator.ClusterCoordinator', function_args=[], function_kwargs={}, custom_class=None)
...
for (batch_data, labels) in eval_dataset:
    pred = custom_method(
    model(batch_data, training=False), imports='import os;import random;import portpicker;import multiprocessing;import tensorflow as tf', function_to_run='obj(*args, **kwargs)', method_object=eval('model'), object_signature='tf.keras.models.Sequential', function_args=[eval('batch_data')], function_kwargs={'training': eval('False')}, custom_class=None)
    actual_pred = custom_method(
    tf.cast(tf.greater(pred, 0.5), tf.int64), imports='import os;import random;import portpicker;import multiprocessing;import tensorflow as tf', function_to_run='tf.cast(*args)', method_object=None, object_signature=None, function_args=[eval('tf.greater(pred, 0.5)'), eval('tf.int64')], function_kwargs={})
    custom_method(
