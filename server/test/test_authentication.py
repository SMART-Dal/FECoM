...
def test_correctUsernamePassword_runsRequestOnServer():
    result = send_request("import numpy as np", "np.linspace(0,10,20)", username="tim9220", password="qQ32XALjF9JqFh!vF3xY", return_result=True)
    assert result["return"].all() == np.linspace(0,10,20).all()
