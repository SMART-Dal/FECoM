...
class EnergyData():
    ...
    def __energy_in_execution(self, energy_df: pd.DataFrame, start_time: float, end_time: float):
        ...
        energy_in_execution = energy_df.loc[(energy_df["time_elapsed"] >= start_time) & (
            energy_df["time_elapsed"] < end_time)]
    ...
    def __energy_lag(self, energy_df: pd.DataFrame, end_time: float, stable_energy_mean: float, abs_tolerance: float):
        ...
        print(f"ngroups: {energy_lag_df_groups.ngroups}")
        print(f"group keys: {energy_lag_df_groups.groups.keys()}")
        ...
        lag_time = energy_lag_df.iloc[-1]["time_elapsed"] - \
            energy_lag_df.iloc[0]["time_elapsed"]
...
class DataLoader():
    ...
    def get_all_data_files(self) -> list:
        ...
        """
        all_data_files = []
        for data_file in sorted(os.listdir(self.__data_dir)):
